...
class MockProtocolSenderNormalMessage extends Mock implements ProtocolSenderNormalMessage {}
...
void main()
{
  ...
  group('addMessage()', () {
    test('addMessage correctly formats and inserts into queue', () {
      Message msg = Message(messageId: 0, text: "Hello", senderId: 0);
      Chat chat = Chat(chatId: 0, userId: 1);
      when(() => mockProtocolSenderNormalMessage.send(any(), any(), any(), any())).thenAnswer((_) async => OkEvent());
      messagesQueue.addMessage(FakeServerApi(),mockProtocolSenderNormalMessage, msg, chat);
      expect(messagesQueue.getMessagesQueue().length, 1);
      expect(messagesQueue.getMessagesQueue().first.$1.text, "Hello");
    });
  });
  group('sendMessages()', () {
    test("should remove message from queue when send is successful", () async {
        Message msg = Message(messageId: 0, text: "Hello World", senderId: 0);
        Chat chat = Chat(chatId: 0, userId: 1);
        when(() => mockProtocolSenderNormalMessage.send(any(), any(), any(), any())).thenAnswer((_) async => OkEvent());
        messagesQueue.addMessage(FakeServerApi(),mockProtocolSenderNormalMessage, msg, chat);
        await messagesQueue.sendMessages(FakeServerApi(),mockProtocolSenderNormalMessage);
    ...
    test('retry logic waits more that 1 second on failure', () {
      fakeAsync((async) {
        Message msg = Message(messageId: 0, text: "Hello World", senderId: 0);
        Chat chat = Chat(chatId: 0, userId: 1);
        when(() => mockProtocolSenderNormalMessage.send(any(), any(), any(), any())).thenAnswer((_) async => null);
        messagesQueue.addMessage(FakeServerApi(),mockProtocolSenderNormalMessage, msg, chat);
        messagesQueue.sendMessages(FakeServerApi(),mockProtocolSenderNormalMessage);
        async.flushMicrotasks();
        when(() => mockProtocolSenderNormalMessage.send(any(), any(), any(), any())).thenAnswer((_) async => OkEvent());
        async.elapse(const Duration(seconds: 1));
        expect(messagesQueue.getMessagesQueue().isNotEmpty, true,reason: "message should still exist after 1 sec because the sender must not retry again before waiting");
      });
    });
  });
}